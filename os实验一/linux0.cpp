...
customer * read_data_customer()      //读取customer.txt信息  
{
	FILE *fp;
	char word;
	customer *p = NULL;
...
orders *read_data_order()	//读取orders.txt信息 
{
	FILE *fp;
	char word1;
	char word2;
...
lineitem *read_data_lineitem()	//读取lineitem.txt信息 
{
	FILE *fp;
	char word1;
	char word2;
...
result *select(customer *cus, orders *ord, lineitem *lin,char *order_date,char *ship_date)		//选择函数 
{
	result *flag1=NULL;
	result *flag2=NULL;
	int control = 0;
...
int find(char *a,chr*b,char*c,int d)
{
	int i;
	customer *cus;
	orders *ord;
...
int main(int argc,char **argv)
{
	...
	for(i=1;i<limit;i++)
	{
		...
		a=find(argv[4*i+1],argv[4*i+2],argv[4*i+3],t);
	}
	return 0;
}
